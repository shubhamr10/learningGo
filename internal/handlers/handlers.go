...
func (m *Repository) AdminPostShowReservation(w http.ResponseWriter, r *http.Request) {
	...
	if year == "" {
		...
		http.Redirect(w, r, fmt.Sprintf("/admin/reservation-calendars?y=%s&m=%s", year, month), http.StatusSeeOther)
	}
	...
	for _, x := range rooms {
		...
		for name, value := range curMap {
			...
			if val, ok := curMap[name]; ok {
				...
				// rest are just the placeholders for days without blocks.
				if val > 0 {
